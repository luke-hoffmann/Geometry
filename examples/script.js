...
let lights = [new geometry.Light(new colorhandler.ColorHandler(255,0,0),new geometry.Vector(0,0,-1000),.2)];
lights.push(new geometry.Light(new colorhandler.ColorHandler(0,0,255),new geometry.Vector(0,0,1000),.6));
...
const s = ( sketch ) => {
  ...
  sketch.draw = () => {
    ...
    renderer.graph();
    const radius = 2000;
    pos = new geometry.Vector( 0,Math.sin(i)*10000, Math.cos(i)*10000);
    renderer.setSceneLightPos(pos,0);
    pos = new geometry.Vector( Math.sin(i)*10000,0, Math.cos(i)*10000);
